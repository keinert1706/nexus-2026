...
Landing mobile-first para el evento **Nexus 2026**, con formulario de confirmación de asistencia (RSVP), botones para agendar en calendario, y un dashboard de registros en tiempo real. Costo: $0 (Supabase + Netlify, tiers gratuitos).
...
- **`netlify/functions/`** → 3 funciones (mini-programas) que corren en Netlify y son las únicas que hablan con la base de datos. La landing y el dashboard **nunca** tocan Supabase directamente — así ningún dato (emails, teléfonos, alergias) queda expuesto en el código que ve el navegador.
- **`supabase/schema.sql`** → el script que crea la tabla de asistentes en Supabase.

Por eso necesitas configurar **3 cosas** antes de que funcione: Supabase (la base de datos), GitHub (donde vive el código) y Netlify (donde se publica el sitio).

---

## Paso 1 — Crear el proyecto en Supabase

1. Ve a [supabase.com](https://supabase.com) y crea una cuenta gratuita (con tu email, ej. `analista.contenido@ceramicaitalia.com`).
2. Clic en **"New project"**. Ponle un nombre, ej. `nexus-2026`, elige una contraseña de base de datos (guárdala, no la necesitarás de nuevo para este proyecto) y una región cercana a Colombia/Latam. Espera 1-2 minutos a que se cree.
3. En el menú izquierdo, ve a **SQL Editor** → **New query**.
4. Abre el archivo [`supabase/schema.sql`](supabase/schema.sql) de este proyecto, copia **todo** su contenido, pégalo en el editor de Supabase, y haz clic en **Run**. Esto crea la tabla `asistentes` con todas sus reglas de seguridad.
5. Ve a **Project Settings** (ícono de engranaje) → **API**. Ahí vas a ver dos valores que necesitarás en el Paso 3:
   - **Project URL** → esto es tu `SUPABASE_URL`
   - **service_role key** (en la sección "Project API keys", **no** la "anon" / "public") → esto es tu `SUPABASE_SERVICE_ROLE_KEY`

   ⚠️ La `service_role key` es secreta — nunca la compartas ni la pegues en el código, solo va en las variables de entorno de Netlify (Paso 3).
...
4. Netlify va a detectar automáticamente la configuración (viene incluida en `netlify.toml`). No necesitas cambiar nada en "Build settings". Antes de darle a "Deploy", busca la sección **"Environment variables"** (o entra después en **Site configuration → Environment variables**) y agrega estas 3:

   | Key | Value |
   |---|---|
   | `SUPABASE_URL` | el Project URL que copiaste en el Paso 1 |
   | `SUPABASE_SERVICE_ROLE_KEY` | el service_role key que copiaste en el Paso 1 |
...
2. Cambia lo que necesites:
   ...
   - `eventStartISO` / `eventEndISO` → la fecha/hora real que se usa para los botones de calendario (formato `AAAA-MM-DDTHH:MM:SS-05:00`). **Actualiza esto junto con `dateLabel`** para que el calendario coincida con lo que dice la landing.
   - `menuOptions` → los 3 nombres y descripciones de menú (cuando me los pases, o tú mismo directamente aquí).
...
- Conteo por opción de menú y por tipo de alergia.
...
- **Seguridad de los datos**: la tabla de Supabase tiene Row Level Security activado y **sin ninguna política pública** — es inaccesible directamente desde el navegador. Todo el acceso (guardar RSVP, ver el dashboard) pasa por las Netlify Functions, que usan la `service_role key` solo del lado del servidor.
...
- **Nombres de menú**: están como "Opción A / B / C" en `js/config.js` — pásamelos cuando los tengas y los actualizo (o los editas tú mismo siguiendo la guía de arriba).
